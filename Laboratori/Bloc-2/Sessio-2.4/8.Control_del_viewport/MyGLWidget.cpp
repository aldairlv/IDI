...
void MyGLWidget::initializeGL(){
    ...
    verd = 0.7;
    blau = 1.0;
    glClearColor(vermell, verd, blau, 1.0); // defineix color de fons (d'esborrat)
    ...
    emit enviarRed(vermell*255);
    emit enviarGreen(verd*255);
    emit enviarBlue(blau*255);
...
// useu els paràmetres que considereu (els que hi ha són els de per defecte)
  glViewport (0, 0, ample*(float(factorX)/100.0f), alt*(float(factorY)/100.0f));
  ...
  switch (event->key()) {
    ...
    case Qt::Key_Z: {
      if(not ortogonal) fov = (fov/2.0f - float(M_PI)/180.0f)*2.0f;
         ...
         bottom *= 0.9f;
         top *= 0.9f;
         //cout << "l: " << left << "  r: " << right << "  b: " << bottom << "  t: " << top << endl;
      }
      projectTransform();
      break;
    }
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  {
    // Fem la rotació
    //cout << "e->x(): " << e->x() <<  "  xClick(): " << xClick << "  variacio: " << (e->x() - xClick) << endl;
    angleY += (e->x() - xClick) * M_PI / ample;
    angleX += (yClick - e->y()) * M_PI / alt;
    //cout << "angleY: " << angleY/(float(M_PI)/180) << "   angleX: " << angleX/(float(M_PI)/180)  << endl;
    viewTransform ();
  }
...
void MyGLWidget::zoom(int z) {
    ...
    if(not ortogonal) fov = (fovIni/2.0f - (float(M_PI)/180.0f)*v)*2.0f;
        ...
        //cout << "v: " << v << endl;
        if (z < 0){
            left = -radiEsfera*pow(0.9f,-v);
            right = radiEsfera*pow(0.9f,-v);
            bottom = -radiEsfera*pow(0.9f,-v);
            top = radiEsfera*pow(0.9f,-v);
        }
        else if( z > 0) {
            left = -radiEsfera*pow(1.1f,v);
            right = radiEsfera*pow(1.1f,v);
            bottom = -radiEsfera*pow(1.1f,v);
            top = radiEsfera*pow(1.1f,v);
        }
        else {
            left = -radiEsfera;
            right = radiEsfera;
...
void MyGLWidget::cambiarEscala(int e) {
    makeCurrent();
    spinEscala = e;
    escala = 1.0 + 0.05*e;
   /*
    float s = float(e)/1.0f;
    if (e > 0){
        escala = 1.0 + 0.05*s;
    }
    else if (e < 0) {
...
void MyGLWidget::cambiarAngleY(int y) {
     makeCurrent();
     angleY = y*(float(M_PI)/180);
     viewTransform ();
     update(); 
}

void MyGLWidget::cambiarAngleX(int x) {
     makeCurrent();
     angleX = x*(float(M_PI)/180);
     viewTransform ();
     update();	
}
/*
void MyGLWidget::cambiarColorDeFons(int r, int g, int b) {
     makeCurrent();
     glClearColor(float(r)/255.0f,float(g)/255.0f,float(b)/255.0f, 1.0); // defineix color de fons (d'esborrat)
     update();
}
*/

void MyGLWidget::guardarRed(int r) {
    makeCurrent();
    vermell = float(r)/255.0f;
    setColorDeFons();
    update();
}
void MyGLWidget::guardarGreen(int g) {
    makeCurrent();
    verd = float(g)/255.0f;
    setColorDeFons();
    update();
}
void MyGLWidget::guardarBlue(int b) {
    makeCurrent();
    blau = float(b)/255.0f;
    setColorDeFons();
    update();
}
void MyGLWidget::setColorDeFons() {
    glClearColor(vermell, verd, blau, 1.0);
}

void MyGLWidget::guardarFactorX(int x) {
    makeCurrent();
    factorX = x;
    update();
}
void MyGLWidget::guardarFactorY(int y) {
    makeCurrent();
    factorY = y;
    update();
}
